...
<div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0 mb-6">
    ...
    <form action="{{ route('admin.users.index') }}" method="GET" class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Tìm tên hoặc email..." class="px-4 py-2.5 rounded-2xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-sm w-full sm:w-64">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-50 text-sm">
                @forelse($users as $user)
                    <tr class="hover:bg-gray-50/30 transition-colors">
                        ...
                        <td class="p-4">
                            ...
                            @else
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700">User</span>
                            @endif
                        </td>
                        ...
                        <td class="p-4 text-right">
                            <div class="inline-block text-left relative">
                                ...
                                <div id="dropdown-{{ $user->id }}" class="action-dropdown absolute right-0 mt-1.5 w-48 bg-white border border-gray-150 rounded-2xl shadow-xl z-30 hidden overflow-hidden divide-y divide-gray-50 text-left">
                                    <div class="py-1">
                                        <button type="button" onclick="openEditModal({{ $user->id }}, '{{ addslashes($user->name) }}', '{{ addslashes($user->email) }}', '{{ $user->role }}')" class="w-full text-left px-4 py-2.5 text-xs font-bold text-cyan-600 hover:bg-cyan-50 transition-colors flex items-center space-x-2">
                                            <span>✏️</span>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="p-8 text-center text-gray-400">Không có dữ liệu người dùng</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<!-- Create User Modal -->
<div id="create-user-modal" class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center hidden z-50 p-4">
        ...
        <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select name="role" class="w-full px-4 py-2.5 rounded-2xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-sm bg-white">
                    <option value="user">User thường (Merchant)</option>
...
<!-- Edit User Modal -->
<div id="edit-user-modal" class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center hidden z-50 p-4">
    <div class="bg-white rounded-3xl max-w-md w-full p-6 shadow-xl border border-gray-100 space-y-4">
        <div class="flex items-center justify-between">
